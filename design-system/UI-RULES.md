# Supremacy Live — UI rules (condensed)

The client's visual language is **Command Console v1.0**. Full spec in `HANDOFF.md` (same
folder); token layer is `public/tokens.css`. This file is the short version — read it before
writing any UI, every time, not just the first time.

This file used to live at the repo root as `CLAUDE.md`. It moved here because `CLAUDE.md` is now
the whole-repo entry point (scope, process, dos/don'ts, pre-commit checklist) — see
[`../CLAUDE.md`](../CLAUDE.md). Nothing about the rules below changed, only where they live.
